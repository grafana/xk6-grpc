...
func (s *featureExplorerServer) GetFeature(ctx context.Context, point *Point) (*Feature, error) {
	log.Printf("GetFeature called with: %+v\n", point)
...
func (s *featureExplorerServer) ListFeatures(rect *Rectangle, stream FeatureExplorer_ListFeaturesServer) error {
	log.Printf("ListFeatures called with: %+v\n", rect)
...
func (s *routeGuideServer) RecordRoute(stream RouteGuide_RecordRouteServer) error {
	log.Println("RecordRoute called")
	...
	for {
		...
		if err == io.EOF {
			...
			log.Println("RecordRoute finished, sending summary")
